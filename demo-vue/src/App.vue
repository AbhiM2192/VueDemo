    ...
    <div>
        ...
        <NamedSlotsDemo>
            ...
            <template v-slot:default>
                <h2>Content with name so Default Slot</h2>
                <img src="https://picsum.photos/200" alt="Default Image" />
            </template>
            <template v-slot:footer>
                <h2>FooterContent goes here..</h2>
            </template>
        </NamedSlotsDemo>

        <SlotsDemo>Card Content</SlotsDemo>
        <SlotsDemo><h1>Card Content 2</h1></SlotsDemo>
        <CustomVmodel v-model="customName" />
        ...
        <ul
            v-for="(name, index) in tech"
            :key="name"
            :style="{ listStyleType: 'none' }"
        >
            <li>{{ index + 1 }} - {{ name }}</li>
        </ul>
        <h2 v-for="(h, index) in hero" :key="h.tile">
            {{ index }} . {{ h.tile }} ---> {{ h.name }}
        </h2>
        ...
        <h2 v-for="(value, key, index) in info" :key="value">
            {{ index }} .. {{ key }} .. {{ value }}
        </h2>
...
import CustomVmodel from "./components/CustomComponentVmodel";
import SlotsDemo from './components/SlotsDemo';
import NamedSlotsDemo from "./components/NamedSlotsDemo";
import SlotPropsDemo from "./components/SlotPropsDemo";
export default {
    name: "App",
    components: {
        componentA,
    ...
    data() {
        return {
            ...
            hero: [
                { name: "Bruce Wayne", tile: "Batman" },
                { name: "Tony Stark", tile: "IronMan" },
                { name: "Chris Hemsworth", tile: "Thor" }
            ],
            actors: [
                {
                    name: "Christian Bale",
